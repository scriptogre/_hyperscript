/**
 * When Feature - Reactive data binding
 *
 * Parses: when <expression> [or <expression>]* changes <commands>
 * Executes: Automatically re-runs commands whenever any watched expression's
 *           value changes. Uses Runtime.createEffect for automatic dependency
 *           tracking — no manual signal declarations needed.
 *
 * Supports watching:
 *   - Global variables: when $username changes ...
 *   - Element variables: when :count changes ...
 *   - Compound expressions: when ($x + $y) changes ...
 *   - Multiple expressions: when $x or $y changes ...
 *   - Attributes: when @data-title changes ...
 *   - Properties: when <input/>'s value changes ...
 *
 * The `it` keyword inside the body refers to the new value of the
 * expression that triggered the change.
 */
